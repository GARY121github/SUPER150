...
The operations can be either dividing the number by 2 only if the number is even or you can add or subtract 1 only if
the number is odd.
More Precisely:
...
public class Number_of_Steps_to_reach_1 {
    public static void main(String[] args) {
        ...
        int n = sc.nextInt();
        System.out.println(stepsToReach1(n));
    }
    private static int stepsToReach1(int n){
        if(n == 1){
            return 0;
        }

        if(n % 2 == 0){
            return stepsToReach1(n/2) + 1;
        }
        else{
            int f1 = stepsToReach1(n+1);
            int f2 = stepsToReach1(n-1);
